...
test_pbmc_tiny <- pbmc_tiny
test_pbmc_tiny@meta.data$groups <- "g1"
test_pbmc_tiny@meta.data$groups[7:15] <- "g2"
test_pbmc_tiny@meta.data$seurat_clusters <- 0
test_pbmc_tiny@meta.data$seurat_clusters[1] <- 1
test_pbmc_tiny@meta.data$seurat_clusters[15] <- 1
test_pbmc_tiny@meta.data$cell_types <- "typeA"
test_pbmc_tiny@meta.data$cell_types[3:5] <- "typeB"
test_pbmc_tiny@meta.data$cell_types[15] <- "typeB"

test_that("has_exclusive_idents works in base case", {
  expected_warning <- "Defaulting to general marker analysis"
  expect_warning(has_exclusive_idents(seu = test_pbmc_tiny, cond = "groups",
                                  idents = "seurat_clusters"), expected_warning)
  expect_true(suppressWarnings(has_exclusive_idents(seu = test_pbmc_tiny,
