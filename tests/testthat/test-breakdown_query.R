...
pbmc_tiny@meta.data$seurat_clusters <- c(rep(0, 5), rep (1, 5), rep (2, 5))
...
test_that("breakdown_query works in simple case", {
  ...
  output_df <- signif(breakdown_query(pbmc_tiny, query), 2)
...
test_that("breakdown_query gives warning if any query genes are not found", {
  ...
  expected_df <- c(0.33, 0.27, 0.33)
  names(expected_df) <- query
  expect_warning(breakdown_query(pbmc_tiny, missing_query), "NOEXPA, NOEXPB")
  output_df <- suppressWarnings(signif(breakdown_query(pbmc_tiny,
                                                       missing_query), 2))
  expect_equal(output_df, expected_df)
})
...
test_that("breakdown_query works with query of length one", {
  ...
  output_df <- signif(breakdown_query(pbmc_tiny, single_query), 2)
  expect_equal(output_df, expected_df)
})
